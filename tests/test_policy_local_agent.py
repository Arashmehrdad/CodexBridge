...
from codexbridge.policy.models import CanonicalPermissionTier, PolicyDecisionValue, PolicyEvaluationRequest


def test_local_agent_routes_policy_evaluation_and_pending_approvals(tmp_path: Path) -> None:
    ...
    assert pending.policy_result[0]["approval_request_id"] == evaluation.policy_result["approval_request_id"]
    assert evaluation.command_result is None
    assert evaluation.job_result is None


def test_local_agent_can_show_approve_and_deny_without_execution(tmp_path: Path) -> None:
    policy_engine = PolicyEngine(approvals_dir=tmp_path / "runs" / "approvals")
    ...
    shown = orchestrator.handle_task(f"show approval request {result.approval_request_id}")
    approved = orchestrator.handle_task(f"approve request {result.approval_request_id} as chatgpt")
...
def test_local_agent_does_not_route_edit_tasks_through_policy_execution(tmp_path: Path) -> None:
    result = LocalAgentOrchestrator(policy_engine=PolicyEngine(approvals_dir=tmp_path / "runs" / "approvals")).handle_task("fix and refactor policy engine")
...
def test_policy_modules_do_not_call_forbidden_systems() -> None:
    source = "\n".join(path.read_text(encoding="utf-8") for path in Path("codexbridge/policy").glob("*.py"))
