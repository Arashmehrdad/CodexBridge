...
**Status:** **prepared for owner review; not authorised or executed.** The authoritative gate is [`docs/PUBLIC_CAPABILITY_METADATA_1_GATE_2026-07-29.md`](docs/PUBLIC_CAPABILITY_METADATA_1_GATE_2026-07-29.md).

This proposed lane addresses the misleading process-wide `schema_hash` field without silently redefining a public contract used by all 32 gateways. It first inventories every producer and consumer, preserves the legacy meaning explicitly, and permits only the smallest additive compatibility-safe route to expose effective gateway contract identity from the same schemas used by discovery and runtime validation.

The lane must prove correct hash movement for deliberate schema changes, stability for implementation-only changes, per-operation isolation, positive and stale connector convergence, localhost/public-tunnel agreement, and compatibility across the full gateway inventory. It does not authorise memory or other gateway-behaviour changes, connector credentials or internals, deployment, or removal/reinterpretation of `schema_hash` without measured evidence and explicit owner acceptance.
