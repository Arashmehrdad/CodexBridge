...
Two Gate C proof items remain **unexecuted, not failed**: `direct ChatGPT MCP` and `direct Codex MCP` require driving external MCP clients. Closing the proof list needs those two clients exercised directly. Separately, one task (`task_20260727T200608Z_8719fe3d9754`) is `uncertain`/`unresolved` after a malformed controller request omitted `working_directory`; the system failed closed correctly, and resolving it needs a quarantine-disposition decision that Gate C excludes.
