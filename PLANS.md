...
**Status:** Gate A implemented and owner-accepted at `e306d5c9ef0826967c28b63db465b64c1dc0efa2`. **Gate B executed, owner-reviewed, and accepted on 2026-07-27**: ProjectScope v1 schema is live, empty, and inactive. **Gate C is prepared but remains unapproved and non-executable.** `GATE-C-PREREQ-1` was implemented at `761653b` and owner-reviewed, but is **not yet accepted**: reusing one idempotency key with a changed disposition, reason, or successor silently replays the first adjudication instead of rejecting the conflicting request. The focused review is recorded in [`docs/gate-c-prereq-1-owner-review-2026-07-27.md`](docs/gate-c-prereq-1-owner-review-2026-07-27.md). One bounded fix must bind the idempotency key to a normalized request hash and add conflicting-replay tests. **ProjectScope schema v2 activation is not authorized.** The live store and running server remain on schema v1, empty and inactive. Historical disposition, memory integration, Hermes, and push remain unauthorized.
