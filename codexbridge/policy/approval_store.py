...
class ApprovalStore:
    ...
    def create(self, request: ApprovalRequest) -> ApprovalRequest:
        self._path(request.approval_request_id).write_text(request.model_dump_json(indent=2), encoding="utf-8")
        return request
    ...
    def list_pending(self) -> list[ApprovalRequest]:
        return [item for item in self.list_all() if item.status == ApprovalStatus.PENDING]

    def list_all(self) -> list[ApprovalRequest]:
        records = []
        for path in self.approvals_dir.glob("*.json"):
            records.append(ApprovalRequest.model_validate_json(path.read_text(encoding="utf-8")))
        records.sort(key=lambda item: item.created_at)
        return records

    def record_decision(self, approval_request_id: str, *, decided_by: str, approved: bool, notes: str = "") -> ApprovalDecision:
        ...
        self.create(record)
        return ApprovalDecision(approval_request_id=approval_request_id, status=record.status, decided_by=decided_by, decided_at=record.decided_at, decision_notes=notes)
