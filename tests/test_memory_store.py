...
def store(tmp_path: Path, *, block_sensitive: bool = True) -> ProjectMemoryStore:
    return ProjectMemoryStore(tmp_path / "runs" / "memory" / "project_memory.sqlite3", block_sensitive=block_sensitive)
...
def test_create_get_update_archive_memory_record(tmp_path: Path) -> None:
    ...
    assert memory_store.list_records(include_archived=True)[0].memory_id == created.memory_id


def test_list_by_type_repo_project_and_tag(tmp_path: Path) -> None:
    memory_store = store(tmp_path)
    memory_store.create(MemoryRecord(memory_type=MemoryType.DECISION, project_key="p1", repo_name="repo", title="D1", tags=["decision"], content="Use sqlite"))
    memory_store.create(MemoryRecord(memory_type=MemoryType.STATIC, project_key="p2", repo_name="other", title="S1", tags=["static"], content="Note"))
...
def test_simple_search_and_recent_order(tmp_path: Path) -> None:
    memory_store = store(tmp_path)
    older = memory_store.create(MemoryRecord(memory_type=MemoryType.STATIC, title="Old", content="alpha"))
    newer = memory_store.create(MemoryRecord(memory_type=MemoryType.STATIC, title="New", content="beta alpha"))

    result = memory_store.search("alpha")
    assert result.total == 2
    assert memory_store.recent(limit=2)[0].memory_id == newer.memory_id
    assert {record.memory_id for record in result.records} == {older.memory_id, newer.memory_id}


def test_duplicate_source_or_content_is_deduplicated(tmp_path: Path) -> None:
    memory_store = store(tmp_path)
    first = memory_store.create(MemoryRecord(memory_type=MemoryType.RUN, title="Run", content="same", source_kind="run", source_id="1"))
    duplicate = memory_store.create(MemoryRecord(memory_type=MemoryType.RUN, title="Run", content="same", source_kind="run", source_id="1"))
...
def test_secret_like_content_is_blocked_or_redacted(tmp_path: Path) -> None:
    with pytest.raises(ValueError):
        store(tmp_path).create(MemoryRecord(memory_type=MemoryType.STATIC, title="Secret", content="api key = abc123"))

    redacting_store = store(tmp_path / "redact", block_sensitive=False)
    record = redacting_store.create(MemoryRecord(memory_type=MemoryType.STATIC, title="Token", content="token=abc123"))
...
def test_validation_recipes_known_commands_architecture_notes(tmp_path: Path) -> None:
    memory_store = store(tmp_path)
    memory_store.create(MemoryRecord(memory_type=MemoryType.STATIC, title="Recipe", content="python -m pytest -q", tags=["validation_recipe", "known_command"], repo_name="repo"))
    memory_store.create(MemoryRecord(memory_type=MemoryType.STATIC, title="Arch", content="Layered modules", tags=["architecture"], repo_name="repo"))
