...
- No production `LocalAgentOrchestrator` construction site remains without durable application context; no-context instances are test-only. The orchestrator no longer creates an implicit `LocalAgentCommandRunner`, and accepted project-command tasks now require either durable context or an explicitly injected compatibility runner. Local-coding and supervisor flows enforce the same construction rule. Implementation commits: `36729ba16d77d03e388a1879a44ccfc0c6b8e4c8` and `da0c1d8b997975162b35814e0acfcbbe249dfc56`.
- Focused validation passed: `tests/test_local_agent_runner.py` 16 passed; `tests/test_local_coding_manager.py` 11 passed; `tests/test_supervisor_upgrade_flow.py` 8 passed.
- Next unit: remove the now test-only direct-subprocess `LocalAgentCommandRunner` implementation and its standalone helper, migrate explicit compatibility tests to the durable adapter or test doubles, and then run adjacent local-agent/local-coding/supervisor suites.
