...
from codexbridge.memory.importers import import_agents_md, import_command_result, import_job_result, import_pulse_manifest, import_runs
...
def test_command_result_importer_stores_concise_artifact_memory(tmp_path: Path) -> None:
    ...
    result_path.write_text(json.dumps({"run_id": "run1", "command_id": "pytest", "status": "success", "exit_code": 0, "duration_seconds": 1.0, "stdout_path": str(result_dir / "stdout.txt"), "stderr_path": str(result_dir / "stderr.txt"), "result_path": str(result_path)}), encoding="utf-8")
...
def test_job_result_importer_stores_job_memory_with_paths_and_status(tmp_path: Path) -> None:
    job_dir = tmp_path / "runs" / "jobs" / "job1"
    job_dir.mkdir(parents=True)
    result_path = job_dir / "result.json"
    result_path.write_text(json.dumps({"job_id": "job1", "job_profile": "dummy_success", "status": "completed", "exit_code": 0, "duration_seconds": 2, "failure_summary": "", "next_recommended_action": "review", "artifact_paths": []}), encoding="utf-8")
...
def test_pulse_manifest_importer_stores_artifact_memory_without_sending(tmp_path: Path) -> None:
...
def test_import_runs_deduplicates_and_bounds_files(tmp_path: Path) -> None:
    ...
    result_dir.mkdir(parents=True)
    result_path = result_dir / "result.json"
    result_path.write_text(json.dumps({"run_id": "run1", "command_id": "pytest", "status": "success"}), encoding="utf-8")
    memory_repo = repo(tmp_path)
