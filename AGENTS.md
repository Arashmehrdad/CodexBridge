...
`V3-1A-FOUNDATION-1` and `V3-1A-ADAPTER-CONTRACT-1` are accepted and closed after independent audits. `V3-1A-PROCESS-IDENTITY-1` is implemented but not accepted after independent audit. The current corrective subgate is `V3-1A-CANCELLATION-AUTHORITY-1`: identity-proven canonical cancellation, fail-closed environment additions, contained attachment failures, and kernel-backed Windows zero-descendant proof. The owner-approved contract forbids terminating a live PID without an exact matching recorded start identity; cancellation remains pending and the repository lock remains held when ownership is unprovable. It may not launch Claude Code or Codex, use provider accounts, send live provider prompts, add public worker gateways or task commands, expose Soma MCP to workers, or activate production behavior.
