...
- Live replacement smoke evidence on 2026-07-17 passed `typecheck`, `vitest` (6 files, 20 tests), and `frontend_validate`. The exact `eslint` replacement invoked ESLint 10.6.0 but failed because Windows denied access to the existing `D:/Github/Andia_Beauty/.pytest_cache` directory (`EPERM` during `scandir`). No active profile was removed. Removal remains gated on a reviewed non-destructive ACL or exclusion resolution and a successful rerun of the exact ESLint replacement.
