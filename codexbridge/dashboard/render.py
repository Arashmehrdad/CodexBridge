...
def render_dashboard_html(summary: DashboardSummary) -> str:
    sections = [
        ("Overall health", [f"ok={summary.health.ok}", f"read_only={summary.health.read_only}", f"runs_dir={summary.health.runs_dir}"]),
        ("Recent command runs", _lines(summary.commands)),
        ("Long-running jobs", _lines(summary.jobs)),
        ("Supervisors", _lines(summary.supervisors)),
        ("Pending approvals", _lines([item for item in summary.approvals if item.status == "pending"])),
        ...
        ("Memory overview", [f"records={summary.memory.total_records}", *(_lines(summary.memory.recent))]),
        ("Current repo status", [summary.repo_status.summary or summary.repo_status.status]),
    ]
    body = "\n".join(f"<section><h2>{html.escape(title)}</h2>{_list(items)}</section>" for title, items in sections)
    return (
...
def _lines(items) -> list[str]:
    lines = []
    for item in items:
        artifact = f" artifact={item.artifact_path}" if getattr(item, "artifact_path", None) else ""
        lines.append(f"{item.id} [{item.status}] {item.summary or item.failure_summary or item.error}{artifact}")
    return lines or ["No items."]


def _list(items: list[str]) -> str:
    return "<ul>" + "".join(f"<li><code>{html.escape(str(item))}</code></li>" for item in items) + "</ul>"
