...
def test_readme_documents_actual_server_command() -> None:
    ...
    assert "python -m codexbridge.server --config config.yaml --transport http --host 127.0.0.1 --port 8000 --path /mcp" in readme
...
def test_readme_documents_supervisor_workflow() -> None:
    ...
    assert "resume_supervisor(supervisor_id)` to advance exactly one safe step" in readme
    assert "pause_supervisor(supervisor_id)` only when the supervisor is `queued` or `needs_input`" in readme
