...
**Status:** Gate A implemented and owner-accepted at `e306d5c9ef0826967c28b63db465b64c1dc0efa2`. **Gate B executed, owner-reviewed, and accepted on 2026-07-27**: ProjectScope v1 schema is live, empty, and inactive. **Gate C is prepared for owner identity review but is not approved or executable.** `GATE-C-PREREQ-1` is now **implemented and tested, awaiting owner acceptance** — see [`docs/gate-c-prereq-1-quarantine-adjudication-2026-07-27.md`](docs/gate-c-prereq-1-quarantine-adjudication-2026-07-27.md). It adds ProjectScope schema **v2** with an owner-only quarantine adjudication/supersession path; quarantine remains terminal and no record is ever returned to an active state. **The live store is still at schema v1**: applying v2 is a separate Gate-B-shaped activation and is not authorized. Until then the runtime correctly reports `up_to_date: false` with `project_scope_adjudications` missing, so the pending activation is visible rather than silent. Historical disposition, memory, Hermes, and push remain unauthorized.
