...
def test_job_profile_registry_returns_builtins() -> None:
    ...
    assert {"dummy_success", "dummy_failure", "dummy_timeout", "sleep_short", "disabled_demo"} <= set(profiles)
    assert profiles["dummy_success"].permission_tier == PermissionTier.LONG_RUNNING_NON_DESTRUCTIVE_JOB
