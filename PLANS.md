...
Gate C controller proof is complete across Claude, ChatGPT, and Codex. Codex used the attached CodexBridge Soma plugin for an exact scoped replay and status read with no shell, CLI, subprocess, new reservation, run attempt, or backend execution. This proves that attached plugin route; it does not independently prove the separately installed global `soma` namespace. Separately, one task (`task_20260727T200608Z_8719fe3d9754`) is `uncertain`/`unresolved` after a malformed controller request omitted `working_directory`; the system failed closed correctly, and resolving it needs a quarantine-disposition decision that Gate C excludes.
