...
def router(tmp_path: Path, **kwargs) -> CodexEscalationRouter:
    return CodexEscalationRouter(
        ...
        policy_engine=kwargs.pop("policy_engine", PolicyEngine(approvals_dir=tmp_path / "runs" / "approvals")),
...
def test_local_only_tasks_are_not_escalated(tmp_path: Path) -> None:
    for objective in ("inspect repo", "summarize logs", "run tests", "search memory for failures"):
        result = router(tmp_path).route_escalation(CodexEscalationRequest(objective=objective))
...
def test_edit_create_refactor_fix_are_codex_required_packets(tmp_path: Path) -> None:
    for objective in ("edit source file", "create module for routing", "refactor policy code", "fix failing test after diagnosis"):
        result = router(tmp_path).route_escalation(CodexEscalationRequest(objective=objective, repo_name="repo", repo_path=tmp_path))
...
def test_packet_artifacts_are_written_and_include_required_fields(tmp_path: Path) -> None:
...
def test_packet_blocks_sensitive_context_and_respects_file_size(tmp_path: Path) -> None:
    ...
    result = router(tmp_path, router_config=config).route_escalation(
        CodexEscalationRequest(objective="edit source file", repo_path=tmp_path, relevant_files=[secret, big])
    )

    assert result.status == CodexEscalationStatus.BLOCKED
    assert "API_KEY" in result.sensitivity_flags
    manifest = json.loads(result.artifacts.context_manifest_path.read_text(encoding="utf-8"))
    assert any(item["skipped_reason"] == "file_too_large" for item in manifest["relevant_files"])


def test_huge_logs_are_truncated_not_fully_embedded(tmp_path: Path) -> None:
    config = CodexRouterConfig(codex_router_max_log_bytes=20, codex_router_block_sensitive=False)
    result = router(tmp_path, router_config=config).route_escalation(
        CodexEscalationRequest(objective="edit source file", current_error="x" * 100)
    )

    error_snippet = [item for item in result.packet.relevant_snippets if item.source == "current_error"][0].text
...
def test_memory_context_and_local_model_summary_are_optional(tmp_path: Path) -> None:
    memory_repo = ProjectMemoryRepository(db_path=tmp_path / "runs" / "memory" / "project_memory.sqlite3")
    ...
    builder = CodexContextBuilder(memory_repository=memory_repo, local_model=FakeLocalModel())
    result = router(tmp_path, context_builder=builder).route_escalation(CodexEscalationRequest(objective="edit router architecture"))
...
class FakeCodexClient:
    ...
    def invoke(self, request):
        self.called = True
        return CodexInvocationResult(status=CodexEscalationStatus.COMPLETED, invoked=True, summary="done")


def test_fake_codex_client_called_only_when_invoke_enabled_and_policy_permits(tmp_path: Path) -> None:
    client = FakeCodexClient()
    config = CodexRouterConfig(codex_router_invoke_enabled=True, codex_router_require_policy_approval=False)
    result = router(tmp_path, router_config=config, codex_client=client).route_escalation(
...
def test_router_modules_do_not_introduce_forbidden_integrations() -> None:
    source = "\n".join(path.read_text(encoding="utf-8") for path in Path("codexbridge/codex_router").glob("*.py"))
