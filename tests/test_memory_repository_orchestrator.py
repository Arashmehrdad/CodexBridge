...
from codexbridge.memory.models import MemoryRecord, MemoryType, RepoProfileMemory, ValidationRecipeMemory
from codexbridge.memory.repository import ProjectMemoryRepository


def repo(tmp_path: Path) -> ProjectMemoryRepository:
    return ProjectMemoryRepository(db_path=tmp_path / "runs" / "memory" / "project_memory.sqlite3")


def test_decision_validation_and_repo_profile_memory(tmp_path: Path) -> None:
    memory_repo = repo(tmp_path)
    decision = memory_repo.remember_decision("Use SQLite for first memory store", accepted_by="user")
    recipe = memory_repo.remember_validation_recipe_model(ValidationRecipeMemory(repo_name="repo", commands=["python -m pytest -q", "python -m pip check"]))
    profile = memory_repo.remember_repo_profile(RepoProfileMemory(repo_name="repo", test_command_ids=["pytest"], job_profile_ids=["dummy_success"], notes="CodexBridge repo"))
...
def test_latest_job_run_and_continue_last_task(tmp_path: Path) -> None:
    memory_repo = repo(tmp_path)
    memory_repo.store.create(MemoryRecord(memory_type=MemoryType.RUN, title="Run", content="run summary"))
    job = memory_repo.store.create(MemoryRecord(memory_type=MemoryType.JOB, title="Job", content="job summary"))
...
def test_orchestrator_routes_explicit_memory_tasks(tmp_path: Path) -> None:
    ...
    remembered = orchestrator.handle_task("remember project fact: CodexBridge uses allowlisted commands")
...
def test_orchestrator_continue_last_task_uses_memory_only(tmp_path: Path) -> None:
    memory_repo = repo(tmp_path)
    memory_repo.store.create(MemoryRecord(memory_type=MemoryType.JOB, title="Latest job", content="job completed"))

    result = LocalAgentOrchestrator(memory_repository=memory_repo).handle_task("continue last CodexBridge task")

    assert result.memory_result["title"] == "Latest job"


def test_orchestrator_does_not_route_edit_tasks_to_memory(tmp_path: Path) -> None:
    result = LocalAgentOrchestrator(memory_repository=repo(tmp_path)).handle_task("fix and refactor memory store")
...
def test_memory_modules_do_not_execute_commands_or_call_external_agents() -> None:
    source = "\n".join(path.read_text(encoding="utf-8") for path in Path("codexbridge/memory").glob("*.py"))
