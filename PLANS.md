...
## Active Lane: V3-1B — Kernel of One

**Status:** architecture accepted after correction and repeated adversarial audit. The sole active package is [`V3-1B-SCHEMA-MODELS-1`](docs/V3_1B_SCHEMA_MODELS_1_GATE_2026-08-02.md).

**Goal:** implement only the additive Company Kernel v1 schema, immutable models, deterministic exact identity/hash material, ordered migration registration and fresh/copied-live migration proof.

**Method:** preserve ProjectScope, Task, Run, process, lock, interaction and ResultPublication authority unchanged; add no package execution lifecycle; prove authority ceilings, immutability, idempotent migration, copied-live compatibility and zero backfill before any company behavior.

**Boundaries:** no public company gateway, Company/Mission runtime action, plan/package action, attempt reservation, Task or Run launch, AcceptanceCommit action, projection, `reconcile_one`, real provider, worker-facing Soma MCP, department, scheduled autonomy, external mutation, restart, connector refresh, deployment or push.

**Exit:** accepted schema/models result with focused and broad tests, copied-live evidence, clean local commit and canonical knowledge refresh. Only then may one separately bounded next package be activated.
