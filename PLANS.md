...
- Next unit: classify the active `LocalAgentCommandRunner.run_project_command` call chain across orchestration, local coding, and supervisor flows, then define whether it must migrate to durable project-command runs or unrestricted PowerShell before any removal.
