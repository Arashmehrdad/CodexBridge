...
from .models import CodexConstraint, CodexContextFile, CodexContextSnippet, CodexEscalationPacket, CodexEscalationRequest
from .redaction import detect_sensitive_text, redact_for_codex


class CodexContextBuilder:
    def __init__(self, *, config: CodexRouterConfig | None = None, memory_repository: ProjectMemoryRepository | None = None, local_model=None):
        self.config = config or CodexRouterConfig()
        self.memory_repository = memory_repository
        self.local_model = local_model

    def build_packet(self, request: CodexEscalationRequest, policy_decision: dict) -> tuple[CodexEscalationPacket, list[str], list[str]]:
        ...
        for path in request.relevant_files:
            ...
            if not allowed:
                context_files.append(CodexContextFile(path=resolved, included=False, skipped_reason=reason))
            ...
            if flags and self.config.codex_router_block_sensitive:
                ...
                context_files.append(CodexContextFile(path=resolved, included=False, skipped_reason="sensitive_content", bytes_read=len(data)))
                continue
            if len(data) > self.config.codex_router_max_file_bytes:
                context_files.append(CodexContextFile(path=resolved, included=False, skipped_reason="file_too_large", bytes_read=len(data)))
                skipped.append(f"{resolved}: file_too_large")
                continue
            if flags and self.config.codex_router_redact_sensitive:
                text = redact_for_codex(text)
                sensitivity.extend(flags)
            if context_bytes + len(text.encode("utf-8")) <= self.config.codex_router_max_context_bytes:
                snippets.append(CodexContextSnippet(source=str(resolved), text=text))
                context_bytes += len(text.encode("utf-8"))
                context_files.append(CodexContextFile(path=resolved, included=True, bytes_read=len(data)))
            else:
                context_files.append(CodexContextFile(path=resolved, included=False, skipped_reason="context_budget_exceeded", bytes_read=len(data)))
        ...
        if request.current_error:
            ...
            if len(error_text.encode("utf-8")) > self.config.codex_router_max_log_bytes:
                error_text = error_text.encode("utf-8")[: self.config.codex_router_max_log_bytes].decode("utf-8", errors="ignore") + "\n[truncated]"
            flags = detect_sensitive_text(error_text)
            if flags and self.config.codex_router_redact_sensitive:
                error_text = redact_for_codex(error_text)
            sensitivity.extend(flags)
            snippets.append(CodexContextSnippet(source="current_error", text=error_text))
        ...
        packet = CodexEscalationPacket(
            ...
            current_error=redact_for_codex(request.current_error) if self.config.codex_router_redact_sensitive else request.current_error,
            failure_summary=redact_for_codex(request.failure_summary) if self.config.codex_router_redact_sensitive else request.failure_summary,
            tests_already_run=request.tests_already_run,
            validation_commands=request.validation_commands or self.config.codex_router_default_validation_commands,
            ...
            if not result.records:
                for term in request.objective.split():
                    ...
                    if result.records:
                        break
            return [
                {"memory_id": record.memory_id, "title": record.title, "summary": record.summary, "tags": record.tags}
    ...
    def _local_model_summary(self, request: CodexEscalationRequest) -> str:
        if not self.config.codex_router_use_local_model_summary or self.local_model is None:
            return ""
        try:
