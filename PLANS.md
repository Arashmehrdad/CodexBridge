...
**Status:** architecture/schema foundations, Agent/Worker mechanics, `V3-1B-BOOTSTRAP-PLAN-AUTHORITY-1`, `V3-1B-OUTCOME-ACCEPTANCE-1`, and `V3-1B-PROJECTION-RECONCILE-1` are accepted. [`V3-1B-PUBLIC-COMPANY-GATEWAYS-1`](docs/V3_1B_PUBLIC_COMPANY_GATEWAYS_1_GATE_2026-08-14.md) is active.
...
**Active package outcome:** expose the accepted Company Kernel authorities through strict bounded `company_query` / `company_action` discriminated-union gateways that delegate to the existing internal transactions, preserve exact ProjectScope/authority/version/idempotency rules, and keep public capability claims honest until the separate live activation gate.

**Boundary:** no automatic/model-owned acceptance, live reasoning/provider activation, Codex use without a new explicit owner request, live Company Kernel activation, worker-facing Soma MCP, Role/Assignment/delegation state, scheduled autonomy, external business mutation, deployment or push.

**Next transition after acceptance:** separate controlled live Kernel-of-One activation with restart/refresh, disposable end-to-end proof and legacy-start measurement.
