...
def _rebind_server_ssh_helpers() -> None:
    server_module = sys.modules.get("soma.server")
    commands_module = sys.modules.get("soma.ssh_commands")
    manager_module = sys.modules.get("soma.ssh_profile_manager")
    tools_module = sys.modules.get("soma.ssh_tools")
...
def _rebind_server_job_manager() -> None:
    server_module = sys.modules.get("soma.server")
    job_manager_module = sys.modules.get("soma.job_manager")
...
RELOADABLE_MODULES = {
    "soma.capabilities",
    "soma.command_profiles",
    "soma.config",
    "soma.git_tools",
    "soma.external_fixtures",
    "soma.job_manager",
    ...
    "soma.run_store",
    "soma.ssh_commands",
    "soma.ssh_profile_manager",
    "soma.ssh_tools",
    "soma.ssh_watchdog",
    "soma.transactions",
    "soma.return_loop.atomic_writer",
}
