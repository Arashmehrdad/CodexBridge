...
- `project_command` caller classification found that the synchronous server `run_project_command` wrapper has no runtime callers beyond discovery/server tests and README documentation. Durable workflow `project_command` steps and the independent `LocalAgentCommandRunner.run_project_command` remain active, distinct surfaces and are not part of that first removal.
- Next unit: remove only the synchronous server `run_project_command` compatibility tool and its discovery/server tests and README guidance, while retaining durable workflow steps, typed validation operations, and the independent local-agent runner.
