...
class LocalCodingManager:
    def __init__(
        ...
        *,
        ...
        self.command_runner = command_runner or LocalAgentCommandRunner(runs_dir=self.runs_dir.parent)
    ...
    def prepare_local_edit(self, request: LocalCodingRequest) -> LocalPatchPreview:
        ...
        classification = classify_local_coding_task(request.objective, operations, self.config)
        run_dir = self._run_dir(request.edit_id)
        run_dir.mkdir(parents=True, exist_ok=True)
        self._event(request.edit_id, "local_coding_requested", "Local coding requested", {"objective": request.objective})
        ...
        if not _target_is_readable(request.repo_path, target_path):
            return self._blocked_preview(request, ["path_outside_repo" if not _target_inside_repo(request.repo_path, target_path) else "target_file_missing"])
        ...
        validation = validate_patch(repo_path=request.repo_path, target_file=target_file, original=original, updated=updated, diff=diff, config=self.config)
        ...
        status = LocalCodingStatus.BLOCKED if preview_policy.blocked or apply_policy.human_required else LocalCodingStatus.APPROVAL_REQUIRED
        ...
        proposal = LocalEditProposal(
            ...
            validation_commands=request.validation_commands or self.config.local_coding_default_validation_commands,
        ...
        atomic_write_json(run_dir / "policy_result.json", {"preview": preview_policy.to_dict(), "apply": apply_policy.to_dict()})
        if approval_request_id:
            approval = self.policy_engine.approval_store.get(approval_request_id)
            atomic_write_json(run_dir / "approval_request.json", approval.model_dump(mode="json"))
        ...
        self._write_run(
            LocalCodingRun(
                ...
                approval_request_id=approval_request_id,
                artifact_paths=[run_dir / name for name in ("proposal.json", "preview.diff", "preview.md", "policy_result.json", "approval_request.json", "original_snapshot.txt")],
                created_at=proposal.created_at,
                updated_at=utc_now(),
                audit_event_id=proposal.audit_event_id,
            )
        )
        self._event(request.edit_id, "preview_ready", "Patch preview created", {"status": status.value, "approval_request_id": approval_request_id})
        return LocalPatchPreview(
            ...
            approval_request_path=run_dir / "approval_request.json" if approval_request_id else None,
            diff_summary=f"{validation.changed_lines} changed lines, {validation.changed_bytes} changed bytes",
            unified_diff=diff,
            blocked_reasons=[] if status != LocalCodingStatus.BLOCKED else apply_policy.reasons,
            audit_event_id=proposal.audit_event_id,
        )

    def apply_local_edit(self, edit_id: str, approval_request_id: str) -> LocalPatchApplyResult:
        ...
        proposal = LocalEditProposal.model_validate_json((run_dir / "proposal.json").read_text(encoding="utf-8"))
        if proposal.approval_request_id != approval_request_id:
            return self._apply_blocked(edit_id, "approval_request_mismatch", audit_id)
        approval = self.policy_engine.approval_store.get(approval_request_id)
        if approval.required_approver != "chatgpt" or approval.status != ApprovalStatus.APPROVED:
        ...
        if sha256_text(original) != proposal.original_sha256:
            return self._apply_blocked(edit_id, "original_hash_mismatch", audit_id, target_path)
        updated = apply_operations_to_text(original, proposal.operations)
        diff = unified_diff(original, updated, proposal.target_file)
        validation = validate_patch(repo_path=proposal.repo_path, target_file=proposal.target_file, original=original, updated=updated, diff=diff, config=self.config)
        if not validation.valid:
            return self._apply_blocked(edit_id, "validation_blocked:" + ",".join(validation.blocked_reasons), audit_id, target_path)
        ...
        status = LocalCodingStatus.VALIDATION_PASSED if _validation_success(command_results) else LocalCodingStatus.VALIDATION_FAILED
        ...
        atomic_write_json(run_dir / "apply_request.json", {"edit_id": edit_id, "approval_request_id": approval_request_id})
        atomic_write_json(run_dir / "validation_results.json", {"results": command_results})
        atomic_write_json(run_dir / "apply_result.json", result.to_dict())
        self._update_run(edit_id, status=status, apply_result_path=run_dir / "apply_result.json")
    ...
    def rollback_local_edit(self, edit_id: str) -> LocalPatchRollbackResult:
        ...
        if not (run_dir / "proposal.json").exists():
            return LocalPatchRollbackResult(edit_id=edit_id, status=LocalCodingStatus.FAILED, error="unmanaged_edit", audit_event_id=f"local_coding_{uuid4().hex}")
        proposal = LocalEditProposal.model_validate_json((run_dir / "proposal.json").read_text(encoding="utf-8"))
        ...
        if result.status == LocalCodingStatus.ROLLED_BACK:
            self._update_run(edit_id, status=LocalCodingStatus.ROLLED_BACK, rollback_result_path=run_dir / "rollback_result.json")
        self._event(edit_id, "rollback_completed", "Rollback completed", {"status": result.status.value})
        return result

    def get(self, edit_id: str) -> LocalCodingRun:
        return LocalCodingRun.model_validate_json((self._run_dir(edit_id) / "run.json").read_text(encoding="utf-8"))

    def list(self, limit: int = 20) -> list[LocalCodingRun]:
        runs = [LocalCodingRun.model_validate_json(path.read_text(encoding="utf-8")) for path in self.runs_dir.glob("*/run.json")]
    ...
    def _operations_from_request(self, request: LocalCodingRequest) -> list[LocalPatchOperation]:
        ...
        if self.config.local_coding_use_local_model and not operations and self.local_model is not None:
            try:
    ...
    def _blocked_preview(self, request: LocalCodingRequest, reasons: list[str], diff: str = "") -> LocalPatchPreview:
        run_dir = self._run_dir(request.edit_id)
        audit_id = f"local_coding_{uuid4().hex}"
        atomic_write_json(run_dir / "proposal.json", {"edit_id": request.edit_id, "status": LocalCodingStatus.BLOCKED.value, "blocked_reasons": reasons})
        atomic_write_text(run_dir / "preview.diff", diff)
        atomic_write_text(run_dir / "preview.md", "Local coding preview blocked.\n\n" + "\n".join(f"- {reason}" for reason in reasons))
        atomic_write_json(run_dir / "policy_result.json", {"blocked": True, "reasons": reasons})
        self._write_run(
            LocalCodingRun(
                edit_id=request.edit_id,
                ...
                preview_diff_path=run_dir / "preview.diff",
                preview_md_path=run_dir / "preview.md",
                artifact_paths=[run_dir / "proposal.json", run_dir / "preview.diff", run_dir / "preview.md", run_dir / "policy_result.json"],
                created_at=utc_now(),
                updated_at=utc_now(),
                audit_event_id=audit_id,
            )
        )
        self._event(request.edit_id, "preview_blocked", "Patch preview blocked", {"reasons": reasons})
        return LocalPatchPreview(edit_id=request.edit_id, status=LocalCodingStatus.BLOCKED, proposal_path=run_dir / "proposal.json", preview_diff_path=run_dir / "preview.diff", preview_md_path=run_dir / "preview.md", policy_result_path=run_dir / "policy_result.json", diff_summary="blocked", unified_diff=diff, blocked_reasons=reasons, audit_event_id=audit_id)

    def _apply_blocked(self, edit_id: str, error: str, audit_id: str, target_path: Path | None = None) -> LocalPatchApplyResult:
        result = LocalPatchApplyResult(edit_id=edit_id, status=LocalCodingStatus.BLOCKED, target_file=target_path, apply_result_path=self._run_dir(edit_id) / "apply_result.json", error=error, audit_event_id=audit_id)
        atomic_write_json(self._run_dir(edit_id) / "apply_result.json", result.to_dict())
    ...
    def _run_validation(self, proposal: LocalEditProposal) -> list[dict]:
        ...
        for command_id in proposal.validation_commands:
            if command_id not in {"git_status", "pytest", "pip_check"}:
                results.append({"command_id": command_id, "status": "blocked", "error": "unknown_validation_command"})
                continue
            result = self.command_runner.run_project_command(command_id=command_id, repo_name=proposal.repo_name, repo_path=proposal.repo_path)
            results.append(result.to_dict())
        return results

    def _target_path(self, repo_path: Path, target_file: Path) -> Path:
        target = Path(target_file)
        return target.resolve() if target.is_absolute() else (Path(repo_path).resolve() / target).resolve()
    ...
    def _event(self, edit_id: str, action: str, message: str, metadata: dict) -> None:
        append_jsonl(self._run_dir(edit_id) / "events.jsonl", {"timestamp": utc_now(), "edit_id": edit_id, "action": action, "message": message, "metadata": metadata})
...
def _validation_success(results: list[dict]) -> bool:
    if not results:
        return True
    blocked = {CommandRunStatus.BLOCKED.value, CommandRunStatus.PROFILE_MISSING.value, CommandRunStatus.PERMISSION_DENIED.value, CommandRunStatus.REPO_MISSING.value}
    return all(item.get("status") not in blocked and (item.get("exit_code") in {0, None}) for item in results)
...
def _target_is_readable(repo_path: Path, target_path: Path) -> bool:
    return _target_inside_repo(repo_path, target_path) and Path(target_path).exists() and Path(target_path).is_file()
