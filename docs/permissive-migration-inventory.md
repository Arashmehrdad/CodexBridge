...
| Local `project_command` public operation | Durable workflow worker plus typed validation starters | Unrestricted PowerShell for arbitrary commands; retained typed validation operations for safe focused checks | Retain the durable substrate while internal synchronous callers migrate; remove only obsolete public compatibility wrappers. |
| `LocalAgentCommandRunner.run_project_command` | Local-agent task routing, local-coding validation, and supervisor validation | Durable typed project-command operations with a bounded result adapter or explicit workflow state | Active compatibility boundary. Do not remove until immediate-result callers are migrated; current implementation lacks RunStore leases, restart adoption, exact cancellation, repository locks, and protected binary artifacts. |
...
- `LocalAgentCommandRunner.run_project_command` remains a separate synchronous implementation used by local-agent orchestration, local-coding, and supervisor flows. Its migration requires a separate durability and audit decision.
- Classification found three active immediate-result callers: local-agent task routing, local-coding validation, and supervisor validation.
- The runner executes hardcoded `pytest`, `pip_check`, and `git_status` profiles through direct synchronous `subprocess.run`, then writes a separate `runs/local_agent/commands` artifact tree.
- It does not participate in RunStore leases, restart adoption, repository-operation locks, exact process-tree cancellation, or protected binary artifact publication, so it is not an acceptable long-term execution substrate.
- Arbitrary-command use should migrate to unrestricted PowerShell; these three validation callers should instead reuse typed durable project-command operations through a small adapter that either waits within a bounded orchestration step or records explicit workflow state.
- Removal is deferred until that adapter and caller migrations are independently validated.
