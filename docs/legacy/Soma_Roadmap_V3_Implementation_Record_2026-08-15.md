# Soma Roadmap V3 Implementation Record - 2026-08-15

> Preservation snapshot of the pre-cleanup root `PLANS.md` at commit
> `2fff78164869a60e59ac6f9aca4d6f32bcc69fc4`. This file preserves the
> accumulated implementation, validation, and planning history that had grown
> inside the active roadmap. It is historical evidence, not current execution
> authority. Relative links reflect the file's former root location. See
> [`../../PLANS.md`](../../PLANS.md) for the current concise plan.
...
The V3 autonomous-company architecture is owner-accepted after the 2026-07-31 hierarchical-intelligence organisational research closure. `RELIABILITY-1 — EXISTING-RUNTIME-HARDENING` is closed. `V3-1A — INTERACTIVE WORKER SUBSTRATE` is accepted and closed. `V3-1B — KERNEL OF ONE` is now accepted, closed, and live after the corrected architecture/schema foundations, Agent/Worker mechanics, Company/Mission bootstrap and PlanRevision authority, exact OutcomeAcceptance, bounded projections and receipt-only `reconcile_one`, strict public `company_query` / `company_action`, and the controlled live activation proof. The public gateway acceptance is [`docs/V3_1B_PUBLIC_COMPANY_GATEWAYS_1_GATE_2026-08-14.md`](docs/V3_1B_PUBLIC_COMPANY_GATEWAYS_1_GATE_2026-08-14.md) and live activation acceptance is [`docs/V3_1B_CONTROLLED_LIVE_KERNEL_OF_ONE_ACTIVATION_1_GATE_2026-08-14.md`](docs/V3_1B_CONTROLLED_LIVE_KERNEL_OF_ONE_ACTIVATION_1_GATE_2026-08-14.md). The live Company Kernel remains owner-controlled; real reasoning/provider execution remains owner-disabled. The active roadmap lane is now `V3-2 — ROLE-SCOPED CAPABILITY BROKER`. Workers still receive no direct current Soma MCP connection, and scheduled autonomy, later collaboration/organisation outcomes, external business mutation, deployment and push remain inactive.
...
**Status:** ACCEPTED / CLOSED / LIVE. Architecture/schema foundations, Agent/Worker mechanics, bootstrap/PlanRevision authority, exact OutcomeAcceptance, bounded projection/reconciliation, strict public Company gateways, and controlled live Kernel-of-One activation are accepted.

**Accepted mechanics already present:** additive Company Kernel schema v4; immutable Company/Mission/PlanRevision/WorkPackage/Attempt/Acceptance records; trusted configuration-anchored Company/Mission bootstrap; bounded PlanRevision DAG and graph manifest; exact dependency proofs; current-plan reasoning admission through canonical Task; bounded coordinator; provider-neutral reasoning backend contract; protected mutation broker; evidence/FanIn mechanics; exact named-authority OutcomeAcceptance over durable-Run or reasoning publication evidence without synthetic Runs; historical durable AcceptanceCommit proof-hash compatibility. These mechanisms must be reused rather than rebuilt.

**Completed outcome:** strict bounded `company_query` / `company_action` delegate to the accepted internal authorities; source/runtime/public schema are converged at 34 public gateways; Company schema v4 is installed; owner-controlled Company Kernel activation is live; replay/CAS, reasoning-disabled admission refusal, receipt-only reconciliation and legacy-start measurement are proven without synthetic execution facts.

**Boundary after closure:** no automatic/model-owned acceptance, live reasoning/provider activation, Codex use without a new explicit owner request, worker-facing current Soma MCP, Role/Assignment state, scheduled autonomy, external business mutation, deployment or push. Legacy workflow/supervisor retirement remains a separate reviewed change rather than part of V3-1B closure.

**Next transition:** `V3-2 — ROLE-SCOPED CAPABILITY BROKER`, beginning with a positive-allowlist worker-facing authorization boundary. Discovery filtering is not authorization and no worker receives a direct current Soma MCP connection before V3-2 acceptance.
...
**Active lane:** `V3-2 — ROLE-SCOPED CAPABILITY BROKER`. V3-1A and V3-1B are accepted and closed. V3-2 must establish a separate worker-facing positive-allowlist authorization boundary before any worker receives Soma capabilities; the current owner/executive MCP surface remains unchanged.
...
**Closed historical package:** `V3-1A-INTERACTION-FOUNDATION-ARCH-REVIEW-1` and its re-scoped interaction implementation are accepted as part of the closed V3-1A lane.

**Current implementation boundary:** V3-2 capability delegation is active for engineering only. Real providers remain owner-disabled; no worker-facing Soma capability surface, scheduled autonomy, later V3 outcome, deployment or push is active before its own acceptance gate.
