...
- Next unit: thread the live `AppConfig`, config path, and shared `JobManager` into the production local-coding and supervisor manager creation sites, then migrate the general local-agent orchestrator separately and remove the remaining direct subprocess runner only after its callers are durable.
