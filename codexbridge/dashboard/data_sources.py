...
def get_dashboard_summary(
    ...
    *,
    ...
    summary = DashboardSummary(
        ...
        runs=_bounded(_collect_root_runs(runs_dir, limit, max_file_bytes, errors), limit),
        commands=_bounded(_collect_commands(runs_dir, limit, max_file_bytes, errors), limit),
        jobs=_bounded(_collect_jobs(runs_dir, limit, max_file_bytes, errors), limit),
        supervisors=_bounded(_collect_supervisors(runs_dir, limit, max_file_bytes, errors), limit),
        approvals=_bounded(_collect_approvals(runs_dir, limit, max_file_bytes, errors), limit),
        codex_escalations=_bounded(_collect_codex_escalations(runs_dir, limit, max_file_bytes, errors), limit),
        return_loop=_bounded(_collect_return_loop(runs_dir, limit, max_file_bytes, errors), limit),
        local_coding=_bounded(_collect_local_coding(runs_dir, limit, max_file_bytes, errors), limit),
        memory=_collect_memory(runs_dir, limit, memory_repository, errors) if include_memory else DashboardMemorySummary(),
        repo_status=_collect_repo_status(runs_dir, limit, max_file_bytes, errors) if include_repo_status else DashboardRepoStatusSummary(status="disabled"),
    )
...
def _collect_commands(runs_dir: Path, limit: int, max_file_bytes: int, errors: list[str]) -> list[DashboardCommandSummary]:
    items = []
    for path in _safe_glob(runs_dir / "local_agent" / "commands", "*/result.json", limit, max_file_bytes, errors):
        data = _read_json(path, runs_dir, max_file_bytes, errors)
        if data is None:
            continue
        items.append(
...
def _collect_jobs(runs_dir: Path, limit: int, max_file_bytes: int, errors: list[str]) -> list[DashboardJobSummary]:
    items = []
    for path in _safe_glob(runs_dir / "jobs", "*/result.json", limit, max_file_bytes, errors):
        data = _read_json(path, runs_dir, max_file_bytes, errors)
        if data is None:
            continue
        ...
        items.append(
            DashboardJobSummary(
                ...
                artifact_path=path,
                summary=_safe_text(str(job.get("summary") or job.get("job_profile") or "")),
                failure_summary=_safe_text(str(job.get("failure_summary") or "")),
                next_recommended_action=_safe_text(str(job.get("next_recommended_action") or "")),
...
def _collect_supervisors(runs_dir: Path, limit: int, max_file_bytes: int, errors: list[str]) -> list[DashboardSupervisorSummary]:
    items = []
    for path in _safe_glob(runs_dir / "supervisors", "*/result.json", limit, max_file_bytes, errors):
        data = _read_json(path, runs_dir, max_file_bytes, errors)
        if data is None:
            continue
        items.append(
            DashboardSupervisorSummary(
                ...
                repo_path=_path_or_none(data.get("repo_path")),
                artifact_path=path,
                summary=_safe_text(str(data.get("local_plan_summary") or data.get("local_inspection_summary") or data.get("objective") or "")),
                failure_summary=_safe_text(str(data.get("failure_summary") or "")),
                next_recommended_action=_safe_text(str(data.get("next_recommended_action") or "")),
...
def _collect_approvals(runs_dir: Path, limit: int, max_file_bytes: int, errors: list[str]) -> list[DashboardApprovalSummary]:
    items = []
    for path in _safe_glob(runs_dir / "approvals", "*.json", limit, max_file_bytes, errors):
        data = _read_json(path, runs_dir, max_file_bytes, errors)
        if data is None:
            continue
        items.append(
...
def _collect_codex_escalations(runs_dir: Path, limit: int, max_file_bytes: int, errors: list[str]) -> list[DashboardCodexEscalationSummary]:
    items = []
    for path in _safe_glob(runs_dir / "codex_escalations", "*/packet.json", limit, max_file_bytes, errors):
        data = _read_json(path, runs_dir, max_file_bytes, errors)
        if data is None:
            continue
        ...
        items.append(
            DashboardCodexEscalationSummary(
                ...
                status="invoked" if result_path.exists() or invocation_path.exists() else "packet_ready",
                created_at=str(data.get("created_at") or ""),
                repo_name=data.get("repo_name"),
                repo_path=_path_or_none(data.get("repo_path")),
                artifact_path=path,
...
def _collect_return_loop(runs_dir: Path, limit: int, max_file_bytes: int, errors: list[str]) -> list[DashboardReturnLoopSummary]:
    items = []
    for root in (runs_dir / "jobs", runs_dir / "supervisors"):
        for path in _safe_glob(root, "*/pulse_manifest.json", limit, max_file_bytes, errors):
            data = _read_json(path, runs_dir, max_file_bytes, errors)
            if data is None:
                continue
            ...
            items.append(
                DashboardReturnLoopSummary(
                    ...
                    readiness="acknowledged" if ack_path.exists() else ("ready" if data.get("ready") else "not_ready"),
...
def _collect_local_coding(runs_dir: Path, limit: int, max_file_bytes: int, errors: list[str]) -> list[DashboardLocalCodingSummary]:
    items = []
    for path in _safe_glob(runs_dir / "local_coding", "*/proposal.json", limit, max_file_bytes, errors):
        data = _read_json(path, runs_dir, max_file_bytes, errors)
        if data is None:
            continue
...
def _collect_memory(runs_dir: Path, limit: int, memory_repository: ProjectMemoryRepository | None, errors: list[str]) -> DashboardMemorySummary:
    try:
        repository = memory_repository or ProjectMemoryRepository(db_path=runs_dir / "memory" / "project_memory.sqlite3")
...
def _collect_repo_status(runs_dir: Path, limit: int, max_file_bytes: int, errors: list[str]) -> DashboardRepoStatusSummary:
    commands = _collect_commands(runs_dir, limit, max_file_bytes, errors)
    for command in commands:
        if command.command_id == "git_status":
            return DashboardRepoStatusSummary(latest_git_status_artifact=command.artifact_path, summary=command.summary, status=command.status)
    return DashboardRepoStatusSummary(status="artifact_missing", summary="No existing git_status artifact found.")


def _safe_glob(root: Path, pattern: str, limit: int, max_file_bytes: int, errors: list[str]) -> list[Path]:
    root = Path(root)
    if not root.exists():
        return []
    paths = []
...
def _read_json(path: Path, runs_dir: Path, max_file_bytes: int, errors: list[str]) -> dict[str, Any] | None:
    try:
...
def _sort_items(items: list[T]) -> list[T]:
    return sorted(items, key=lambda item: item.updated_at or item.ended_at or item.created_at or item.id, reverse=True)
