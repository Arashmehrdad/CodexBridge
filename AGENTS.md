...
`V3-1A-FOUNDATION-1` and `V3-1A-ADAPTER-CONTRACT-1` are accepted and closed after independent audits. `V3-1A-PROCESS-IDENTITY-1`, `V3-1A-CANCELLATION-AUTHORITY-1`, and `V3-1A-CANCELLATION-CLOSURE-1` are implemented but not accepted after independent audits. The current bounded corrective subgate is `V3-1A-LAUNCH-FAILURE-CONTAINMENT-1`: identity-safe fresh-launch cleanup, explicit confirmed-versus-uncertain containment, identity-scoped lease-loss/attachment cleanup, and lock retention when a newly created process may remain active. The owner-approved contract forbids terminating or enumerating a live PID without an exact matching recorded start identity; unprovable containment remains non-terminal and retains or quarantines mutation ownership. The accepted `KILL_ON_JOB_CLOSE`, environment, publication, cancellation-ordering, and public-projection corrections must remain intact. This gate may not launch Claude Code or Codex, use provider accounts, send live provider prompts, add public worker gateways or task commands, expose Soma MCP to workers, or activate production behavior.
