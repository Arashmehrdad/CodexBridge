...
**Active bounded package:** `V3-1A-INTERACTION-COMMANDS-1 — Canonical Controller Interaction and Non-Terminal Waiting`. Its gate is [docs/V3_1A_INTERACTION_COMMANDS_1_GATE_2026-07-31.md](docs/V3_1A_INTERACTION_COMMANDS_1_GATE_2026-07-31.md). It may add canonical `STEER` and `SUPPLY_INPUT` task commands, persisted-before-send interaction records, deterministic stand-in delivery, exact checkpoint/session binding, bounded deadline and expiry semantics, and genuinely non-terminal `AWAITING_CONTROLLER`. Cancellation must win every race, uncertain delivery must never be blindly resent, and ownership must not be released without quiescence proof. Real Claude/Codex launch, provider accounts, live prompts/streams, direct worker MCP access, V3-1B work, production activation, and push remain forbidden.
