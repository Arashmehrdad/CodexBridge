"""Provider-neutral, inert handoff artifact generation.

Generates a bounded artifact for a human to use outside Soma. Nothing in
this package invokes an external model, executable, worker, or fallback, and
the artifact must never be routed through unrestricted PowerShell.
"""
...
class ExternalCoderHandoffGenerator:
    """Route a coding-shaped objective into an inert handoff artifact.

    The only outcomes are informational states and written files for human
    use outside Soma. There is no invocation, worker, adapter, fallback, or
    PowerShell launch path of any kind.
    """
...
def build_handoff_prompt(handoff: ExternalCoderHandoff) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "This is an inert implementation handoff artifact for a human-selected",
            "tool outside Soma. Soma has not executed, launched, supervised, or",
            "selected any coding agent, and this artifact is not a runnable request.",
            "",
            "# Objective",
            handoff.objective,
            "",
