...
class RiskClassifier:
    def classify(self, request: PolicyEvaluationRequest) -> RiskClassification:
        ...
        return RiskClassification(
            ...
            risk_level=risk,
            human_required=tier == CanonicalPermissionTier.T6_HUMAN_ONLY_RISKY_ACTION,
            reasons=[f"classified_{tier.value.lower()}"],
            matched_rules=["metadata_or_text_classification"],
        )
...
def _normalize_tier(value) -> CanonicalPermissionTier | None:
    ...
    if isinstance(value, CanonicalPermissionTier):
        return value
    return LEGACY_PERMISSION_MAP.get(
        str(value),
        CanonicalPermissionTier(str(value))
        if str(value) in CanonicalPermissionTier._value2member_map_
        else None,
    )
...
def _infer_tier(text: str, action_type: str) -> CanonicalPermissionTier:
    ...
    if any(
        term in text for term in ("commit", "private branch", "feature branch push")
    ):
    ...
    return (
        ...
        else CanonicalPermissionTier.T0_READ_ONLY
    )
