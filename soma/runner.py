...
def _codex_child_env() -> dict[str, str]:
    ...
    for key, value in os.environ.items():
        if key.startswith(blocked_prefixes):
            continue
        env[key] = value
    env["SOMA_CONNECTOR_ISOLATION"] = "enabled"
    return env
        ...
        result.update(
            {
                ...
                "connector_isolation_note": (
                    "Soma stripped child-process MCP connector variables where it controls the subprocess environment. "
                    "Pre-dispatch connector errors on the OpenAI side remain outside Soma control."
                ),
            }
        )
        self._write_artifacts(
