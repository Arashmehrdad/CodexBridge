...
def test_cf1_gateway_operation_inventory_is_versioned_and_exact() -> None:
    ...
    assert CF1_GATEWAY_OPERATION_INVENTORY_VERSION == ("cf1.3.gateway-operations.v24")
...
def test_high_cost_operations_preserve_current_behavioral_baseline() -> None:
    ...
    assert "view=full" in supervisor_notifications.notes
    ...
    identity = _entry_for("system_query", "capability_identity")
    assert identity.default_response_bytes == 12 * 1024
    assert identity.maximum_response_bytes == 12 * 1024
    assert "identity" in identity.notes
