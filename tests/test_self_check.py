...
def test_self_check_reports_supervisor_readiness(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("codexbridge.self_check._run", fake_run)
    config = AppConfig(repos={"sample": RepoConfig(path=str(tmp_path))}, config_dir=tmp_path)
    result = run_self_check(config=config, config_path=None, run_live_server=False, run_tests=False)
    ...
    assert resume_prompts["pattern"].endswith("runs\\supervisors\\<supervisor_id>\\resume_prompt.txt") or resume_prompts[
        "pattern"
    ].endswith("runs/supervisors/<supervisor_id>/resume_prompt.txt")


def test_self_check_does_not_expose_webhook_url_values(monkeypatch, tmp_path: Path) -> None:
    def fake_run(command: list[str], cwd: Path, timeout: int = 120) -> dict:
        return {"command": command, "exit_code": 0, "stdout": "", "stderr": "", "ok": True}
    ...
    config = AppConfig(
        ...
        supervisors=SupervisorsConfig(
            notifications=SupervisorNotificationsConfig(
                webhook=SupervisorWebhookNotificationSinkConfig(enabled=True, url_env="SECRET_WEBHOOK_URL")
            )
        ),
    )
    result = run_self_check(config=config, config_path=None, run_live_server=False, run_tests=False)
    ...
    assert result["checks"]["supervisor_config"]["notification_sinks"]["webhook_enabled"] is True
