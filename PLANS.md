...
**Status:** **prepared for owner review; not authorised or executed.** The authoritative gate is [`docs/EXTERNAL_CONSUMER_MEASUREMENT_1_GATE_2026-07-29.md`](docs/EXTERNAL_CONSUMER_MEASUREMENT_1_GATE_2026-07-29.md).

This measurement-only gate freezes an owner-approved consumer universe and collects complete static, existing-evidence and bounded controlled-probe evidence for the six generic knowledge operations, the exported `LocalAgentOrchestrator` and `classify_task` Python symbols, and actual consumption of legacy `schema_hash`. It distinguishes a field being emitted from a client reading it, and “no use found in the measured universe” from a claim of global non-use.

The lane may create only its result, machine-readable consumer matrix and closure update to this plan. It authorises no deprecation, removal, migration, routing or schema change, package-export change, persistent telemetry, hidden client tracking, unapproved filesystem scan, canonical-state mutation, deployment or Roadmap V3 implementation. Evidence that is unavailable or not observable remains explicit rather than being converted into permission to delete.
