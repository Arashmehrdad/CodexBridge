...
**Active final corrective subgate:** `V3-1A-CANCELLATION-CLOSURE-1 — Publication, Identity Capture, and Crash Containment`. Its gate is [docs/V3_1A_CANCELLATION_CLOSURE_1_GATE_2026-07-30.md](docs/V3_1A_CANCELLATION_CLOSURE_1_GATE_2026-07-30.md). The no-raw-PID contract remains binding. For the first provider-capable substrate, Windows Job Objects must use `KILL_ON_JOB_CLOSE` so a Soma crash stops the bounded worker tree; later recovery uses the exact durable provider-session identity. Real Claude/Codex launch, provider accounts, live prompts/streams, public worker gateways, task-command expansion, direct worker MCP access, V3-1B work, production activation, and push remain forbidden.
