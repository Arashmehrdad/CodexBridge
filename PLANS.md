...
**Status:** **owner-authorised and amended to an exact 8-hour window; implementation validated; production activation pending.** The authoritative gate is [`docs/MCP_OPERATION_USAGE_INSTRUMENTATION_1_GATE_2026-07-29.md`](docs/MCP_OPERATION_USAGE_INSTRUMENTATION_1_GATE_2026-07-29.md).

This temporary instrumentation gate records only aggregate counts keyed by Soma's authoritative `gateway.operation` inventory during one exact 8-hour production window. The owner reduced the original 48-hour duration because Soma runs on a personal laptop that cannot reasonably remain continuously available for two days; all other evidence and privacy controls remain binding. It records no request or response content, per-call event, timestamp, client, address, session, project or repository identity. All public operations are counted so the evidence can support future retirement decisions, while the immediate target remains the six generic knowledge operations.

The counter is disabled by default, must expire automatically, and must be removed with all activation configuration before closure. A valid result requires at least 100 counted calls, zero unmapped operations, restart-safe exact counts, unchanged public contract identity, full privacy and concurrency controls, a restored product tree, full-suite validation and healthy localhost/public-tunnel service. The lane authorises no deprecation, routing change, memory write, two-vault migration, permanent telemetry or Roadmap V3 implementation.
