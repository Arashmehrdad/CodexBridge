...
`V3-1A-FOUNDATION-1` and `V3-1A-ADAPTER-CONTRACT-1` are accepted and closed after independent audits. `V3-1A-PROCESS-IDENTITY-1` and its `V3-1A-CANCELLATION-AUTHORITY-1` correction are implemented but not accepted after independent audits. The current final corrective subgate is `V3-1A-CANCELLATION-CLOSURE-1`: contradictory cancellation-proof closure, complete canonical launcher/child identity capture, positive-policy environment extras, honest ResultPublication retry, and Windows Job Objects with `KILL_ON_JOB_CLOSE`. The owner-approved contract forbids terminating a live PID without an exact matching recorded start identity; cancellation remains pending and the repository lock remains held when ownership is unprovable. A Soma controller crash must stop the bounded local worker tree; later recovery uses the exact durable provider-session identity rather than adopting an uncontrolled orphan. This gate may not launch Claude Code or Codex, use provider accounts, send live provider prompts, add public worker gateways or task commands, expose Soma MCP to workers, or activate production behavior.
