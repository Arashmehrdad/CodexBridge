...
**Status:** Gate A implemented and owner-accepted at `e306d5c9ef0826967c28b63db465b64c1dc0efa2`. **Gate B executed, owner-reviewed, and accepted on 2026-07-27**: ProjectScope v1 schema is live, empty, and inactive. **Gate C is prepared but remains unapproved and non-executable.** `GATE-C-PREREQ-1` is **owner-accepted and closed** after the idempotency correction at `22fd884`. **ProjectScope schema v2 activation was owner-approved, executed, and verified on 2026-07-27**: the live store is now at ProjectScope `2 / 2` with the reviewed adjudication table present and empty, scoped writes still disabled, and enforcement still `inactive`. The review package is [`docs/SCOPE_FOUNDATION_1_PROJECT_SCOPE_V2_ACTIVATION_PREPARATION_2026-07-27.md`](docs/SCOPE_FOUNDATION_1_PROJECT_SCOPE_V2_ACTIVATION_PREPARATION_2026-07-27.md), with machine-readable evidence in [`docs/scope-foundation-1-project-scope-v2-activation-preparation-2026-07-27.json`](docs/scope-foundation-1-project-scope-v2-activation-preparation-2026-07-27.json). The activation itself is recorded in [`docs/SCOPE_FOUNDATION_1_PROJECT_SCOPE_V2_ACTIVATION_2026-07-27.md`](docs/SCOPE_FOUNDATION_1_PROJECT_SCOPE_V2_ACTIVATION_2026-07-27.md), with machine-readable evidence in [`docs/scope-foundation-1-project-scope-v2-activation-2026-07-27.json`](docs/scope-foundation-1-project-scope-v2-activation-2026-07-27.json). Against a fresh closed-store baseline of `4,723` runs and `2` tasks, the live apply returned exactly `[2]` then `[]`, added only the reviewed table and index, changed no existing SQLite object, preserved incumbent identities and publication evidence, and left settings `[0, 0]`; the restarted runtime converged with `mismatches: []`. The rollback backup is `runs/scope-v2-activation-20260727T193134Z/soma-pre-v2-backup.sqlite3` (sha256 `aeb034931d48548ab9d1001dfe1390caf612621acb44db3912eee2d2dcddeb04`). Historical disposition, Gate C, memory integration, Hermes, unrelated cleanup, and push remain unauthorized.
...
Direct ChatGPT MCP is now proven by an exact scoped idempotent replay from the connected ChatGPT client: no task, reservation, or backend run was duplicated. One Gate C proof item remains **unexecuted, not failed**: direct Codex MCP. Separately, one task (`task_20260727T200608Z_8719fe3d9754`) is `uncertain`/`unresolved` after a malformed controller request omitted `working_directory`; the system failed closed correctly, and resolving it needs a quarantine-disposition decision that Gate C excludes.
