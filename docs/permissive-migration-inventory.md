...
| Local synchronous `run_project_command` compatibility tool | No runtime callers found; only server/discovery tests and README guidance reference this server wrapper. The independent local-agent method with the same name is a separate implementation. | Durable `run_start` plus typed validation or unrestricted PowerShell | First removable local-command unit: remove only the synchronous server wrapper, its registration/discovery fixtures, server tests, and README guidance. Retain durable workflow `project_command` steps and the local-agent runner. |
...
- The synchronous server `run_project_command(repo_name, command_id)` wrapper is registered only as a compatibility/internal tool. Repository-wide search found no runtime caller; remaining references are its own server tests, MCP discovery fixtures, README guidance, and the implementation itself.
- Durable workflow steps call `JobManager.start_project_command` and retain child-run reservation, leases, publication, cancellation, and restart behavior. They are not replaced by deleting the synchronous wrapper.
- `LocalAgentCommandRunner.run_project_command` is a separate synchronous implementation used by local-agent orchestration, local-coding, and supervisor flows. Its migration requires a separate durability and audit decision and is not included in the first removal.
- The first safe removal therefore deletes only the server compatibility wrapper and its route metadata/tests/docs. Typed validation operations and unrestricted PowerShell remain the accepted public replacements.
