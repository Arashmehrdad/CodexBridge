...
| `LocalAgentCommandRunner.run_project_command` | Test-only explicit compatibility injection; no production construction fallback remains | `DurableProjectCommandRunner` over typed durable project-command operations | Ready for removal. Orchestrator, local-coding, and supervisor paths now require durable application context or an explicitly injected test runner. |
...
- No production constructor creates `LocalAgentCommandRunner` implicitly; no-context command execution fails explicitly, while tests may still inject a runner double.
- The remaining `LocalAgentCommandRunner` implementation and standalone helper are test-only compatibility code that still use direct synchronous `subprocess.run` and the separate `runs/local_agent/commands` artifact tree.
- They are now eligible for removal together with their direct implementation tests; behavior tests should use `DurableProjectCommandRunner` coverage or lightweight injected doubles instead.
