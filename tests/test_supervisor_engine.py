...
from codexbridge.supervisor_engine import FakeJobBackend, SupervisorEngine
from codexbridge.supervisor_store import SupervisorStore


def make_engine(tmp_path: Path) -> tuple[SupervisorEngine, SupervisorStore, FakeJobBackend]:
...
def active_job_id(supervisor: dict) -> str:
    return supervisor["metadata"]["active_child"]["job_id"]


def advance_to_needs_input(engine: SupervisorEngine, jobs: FakeJobBackend) -> dict:
    supervisor = create_supervisor(engine)
    planning = engine.tick(supervisor["supervisor_id"])
    jobs.complete(active_job_id(planning), summary="plan summary", result={"files": ["README.md"]})
...
def test_planning_running_tick_is_idempotent(tmp_path: Path) -> None:
    ...
    assert active_job_id(again) == active_job_id(planning)
    assert len(jobs.jobs) == 1
...
def test_completed_plan_moves_to_needs_input(tmp_path: Path) -> None:
    ...
    assert needs_input["metadata"]["plan_result"]["result"] == {"files": ["README.md"]}
...
def test_implementing_running_tick_is_idempotent(tmp_path: Path) -> None:
    ...
    assert active_job_id(again) == active_job_id(implementing)
    assert len(jobs.jobs) == 2
...
def test_completed_implementation_marks_supervisor_completed(tmp_path: Path) -> None:
    ...
    needs_input = advance_to_needs_input(engine, jobs)
    implementing = engine.approve_plan(needs_input["supervisor_id"], "approved", ["README.md"], [])
    jobs.complete(active_job_id(implementing), summary="implementation summary", result={"changed_files": []})
    completed = engine.tick(needs_input["supervisor_id"])
    assert completed["status"] == "completed"
    assert completed["summary"] == "implementation summary"
    assert completed["metadata"]["implementation_result"]["result"] == {"changed_files": []}
...
def test_plan_failure_marks_supervisor_failed(tmp_path: Path) -> None:
    ...
    supervisor = create_supervisor(engine)
    planning = engine.tick(supervisor["supervisor_id"])
    jobs.fail(active_job_id(planning), error="plan failed")
...
def test_implementation_failure_marks_supervisor_failed(tmp_path: Path) -> None:
    ...
    needs_input = advance_to_needs_input(engine, jobs)
    implementing = engine.approve_plan(needs_input["supervisor_id"], "approved", ["README.md"], [])
    jobs.fail(active_job_id(implementing), error="implementation failed")
...
def test_cancel_active_plan_marks_cancelled(tmp_path: Path) -> None:
    ...
    assert jobs.get(active_job_id(planning)).status == "cancelled"
...
def test_cancel_active_implementation_marks_cancelled(tmp_path: Path) -> None:
    ...
    assert cancelled["status"] == "cancelled"
    assert jobs.get(active_job_id(implementing)).status == "cancelled"
...
def test_terminal_tick_is_noop(tmp_path: Path) -> None:
    ...
    needs_input = advance_to_needs_input(engine, jobs)
    implementing = engine.approve_plan(needs_input["supervisor_id"], "approved", ["README.md"], [])
    jobs.complete(active_job_id(implementing), summary="done")
    completed = engine.tick(needs_input["supervisor_id"])
...
def test_state_survives_store_reload(tmp_path: Path) -> None:
    ...
    jobs.complete(active_job_id(planning), summary="reloaded plan")
