...
PUBLIC_GATEWAY_OPERATION_INVENTORY: Final[
    ...
    _entry(
        ...
        ("events",),
        "codexbridge.server:workflow_query",
        "direct workflow event list",
        json_decode_cost=JsonDecodeCost.BOUNDED_OBJECT,
        pagination=PaginationBehavior.LIMIT_ONLY,
        default_item_limit=100,
        maximum_item_limit=500,
        notes="Event retrieval is limit bounded but has no cursor or byte ceiling.",
    ),
    _entry(
        "workflow_action",
        ("start",),
    ...
    _entry(
        "run_query",
        ("group_status", "group_result"),
        "codexbridge.server:run_query -> codexbridge.parallel_runs:PowerShellGroupManager",
        "direct parallel-group snapshot",
        json_decode_cost=JsonDecodeCost.BOUNDED_OBJECT,
        notes="Nested child summaries can grow without a public byte ceiling.",
    ),
    _entry(
        "run_query",
        ("locks",),
    ...
    _entry(
        ...
        "codexbridge.server:supervisor_query",
        "direct supervisor event or notification list",
        json_decode_cost=JsonDecodeCost.BOUNDED_OBJECT,
        pagination=PaginationBehavior.LIMIT_ONLY,
        default_item_limit=50,
        maximum_item_limit=500,
        notes="Lists are item limited but expose no cursor or byte ceiling.",
    ),
    _entry(
        "supervisor_action",
        ("start",),
    ...
    _entry(
        ...
        ("h4_candles",),
        "codexbridge.server:trading_query",
        "direct completed-candle series",
        pagination=PaginationBehavior.LIMIT_ONLY,
        default_item_limit=200,
        maximum_item_limit=2_000,
        notes="Candle count is bounded but serialized bytes are not.",
    ),
    _entry(
        "trading_query",
        ("historical_ticks",),
    ...
    _entry(
        ...
        "direct trading journal list",
        json_decode_cost=JsonDecodeCost.BOUNDED_OBJECT,
        pagination=PaginationBehavior.LIMIT_ONLY,
        default_item_limit=100,
        maximum_item_limit=1_000,
        notes="The list is item limited but has no cursor or byte ceiling.",
    ),
    _entry(
    ...
    _entry(
        ...
        "direct repository path list",
        pagination=PaginationBehavior.LIMIT_ONLY,
        default_item_limit=500,
        maximum_item_limit=5_000,
        notes="File count is bounded but there is no cursor or byte ceiling.",
    ),
    _entry(
        "repo_query",
        ("read_files",),
