...
from codexbridge.supervisor import LocalSupervisorManager, SupervisorStatus, SupervisorTaskRequest
...
class FakeCommandRunner:
    ...
    def run_project_command(self, *, command_id: str, repo_name=None, repo_path=None, **kwargs):
        ...
        return CommandRunResult(
            ...
            permission_tier=PermissionTier.SAFE_LOCAL_TEST if command_id in {"pytest", "pip_check"} else PermissionTier.READ_ONLY,
            status=status,
            exit_code=exit_code,
...
class FakeCodexRouter:
    def __init__(self, status: CodexEscalationStatus = CodexEscalationStatus.PACKET_READY, *, invoked: bool = False):
        self.status = status
    ...
    def route_escalation(self, request):
        ...
        return CodexRouterResult(
            ...
            invocation_result=CodexInvocationResult(status=self.status, invoked=self.invoked) if self.invoked else None,
            reasons=["fake_router"],
...
def test_local_only_supervisor_completes_without_codex_packet(tmp_path: Path) -> None:
    ...
    result = manager.start_supervised_task(
        SupervisorTaskRequest(supervisor_id="supervisor_local", objective="inspect repo", validation_commands=["git_status"])
    )
    ...
    manifest = ReportManifest.model_validate_json((run_dir / "pulse_manifest.json").read_text(encoding="utf-8"))
...
def test_edit_task_creates_codex_packet_without_invocation_by_default(tmp_path: Path) -> None:
    router = FakeCodexRouter(CodexEscalationStatus.PACKET_READY)
    manager = make_manager(tmp_path, router=router)

    result = manager.start_supervised_task(
        SupervisorTaskRequest(supervisor_id="supervisor_edit", objective="fix failing test", validation_commands=["git_status"])
    )
    ...
    assert (tmp_path / "runs" / "supervisors" / "supervisor_edit" / "codex_packet_ref.json").exists()
...
def test_codex_invocation_flag_is_only_passed_when_enabled(tmp_path: Path) -> None:
    ...
    result = manager.start_supervised_task(
        SupervisorTaskRequest(supervisor_id="supervisor_invoke", objective="fix failing test", validation_commands=["git_status"])
    )

    assert router.calls[0].invoke_codex is True
    assert result.run.codex_invoked is True
    assert result.run.status == SupervisorStatus.COMPLETED
    assert (tmp_path / "runs" / "supervisors" / "supervisor_invoke" / "post_validation_results.json").exists()


def test_policy_blocked_sensitive_task_stops_before_validation_and_codex(tmp_path: Path) -> None:
    runner = FakeCommandRunner()
    router = FakeCodexRouter()
    manager = make_manager(tmp_path, runner=runner, router=router)

    result = manager.start_supervised_task(
        SupervisorTaskRequest(supervisor_id="supervisor_blocked", objective="use API key password to deploy", validation_commands=["git_status"])
    )
    ...
    assert "password" not in (run_dir / "resume_prompt.txt").read_text(encoding="utf-8").lower()


def test_unknown_validation_command_is_rejected_without_execution(tmp_path: Path) -> None:
    runner = FakeCommandRunner()
    manager = make_manager(tmp_path, runner=runner)

    result = manager.start_supervised_task(
        SupervisorTaskRequest(supervisor_id="supervisor_unknown_command", objective="inspect repo", validation_commands=["unknown_command"])
    )
...
def test_cancel_and_resume_are_status_oriented(tmp_path: Path) -> None:
    ...
    manager.start_supervised_task(
        SupervisorTaskRequest(supervisor_id="supervisor_cancel", objective="inspect repo", validation_commands=["git_status"])
    )
    ...
    assert (tmp_path / "runs" / "supervisors" / "supervisor_cancel" / "pulse_manifest.json").exists()


def test_supervisor_package_introduces_no_pulsesender_browser_or_subprocess_imports() -> None:
