...
**Active bounded corrective subgate:** `V3-1A-LAUNCH-FAILURE-CONTAINMENT-1 — Identity-Safe Fresh-Launch Cleanup`. Its gate is [docs/V3_1A_LAUNCH_FAILURE_CONTAINMENT_1_GATE_2026-07-31.md](docs/V3_1A_LAUNCH_FAILURE_CONTAINMENT_1_GATE_2026-07-31.md). It must remove identity-less raw-PID cleanup, distinguish confirmed stop from uncertainty, retain or quarantine locks on unconfirmed launch containment, make all touched post-identity cleanup identity-scoped, and replace weak source/conditional tests with behavioral regressions. Accepted KILL_ON_JOB_CLOSE, environment, publication, cancellation-ordering, and launcher/child projection results must remain intact. Real Claude/Codex launch, provider accounts, live prompts/streams, public worker gateways, task-command expansion, direct worker MCP access, V3-1B work, production activation, and push remain forbidden.
