...
def test_local_agent_routes_explicit_codex_packet_request(tmp_path):
    router = CodexEscalationRouter(policy_engine=PolicyEngine(approvals_dir=tmp_path / "runs" / "approvals"), packet_dir=tmp_path / "runs" / "codex_escalations")
    result = LocalAgentOrchestrator(codex_router=router).handle_task("prepare Codex packet: edit source file")
