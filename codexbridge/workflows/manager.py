...
from codexbridge.run_store import TERMINAL_STATUSES as CHILD_TERMINAL_STATUSES
...
class WorkflowManager:
    ...
    def _launch_worker_process(self, config_path: Path, workflow_id: str, lease_token: str, lease_generation: int) -> int:
        command = [sys.executable, "-m", "codexbridge.workflows.worker", "--config", str(config_path), "--workflow-id", workflow_id, "--lease-token", lease_token, "--lease-generation", str(int(lease_generation))]
