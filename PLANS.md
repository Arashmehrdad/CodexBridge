...
**Architecture status:** the V3 autonomous-company architecture is recorded in [docs/SOMA_ROADMAP_V3_AUTONOMOUS_COMPANY_ARCHITECTURE_2026-07-30.md](docs/SOMA_ROADMAP_V3_AUTONOMOUS_COMPANY_ARCHITECTURE_2026-07-30.md). Claude Opus architectural red-team, Codex repository-feasibility review and owner reconciliation are complete. The accepted reconciliation is [docs/SOMA_V3_ARCHITECTURE_RECONCILIATION_2026-07-30.md](docs/SOMA_V3_ARCHITECTURE_RECONCILIATION_2026-07-30.md).

**Lifecycle policy:** V3 uses progressive lifecycle convergence. No broad preliminary consolidation lane is required. Every V3 lane must consolidate or project the generic lifecycle responsibilities it touches into the canonical task/run plane, preserve irreducible domain state, identify compatibility bridges and retirement conditions, and close with no unexplained increase in canonical authority.

**Corrected sequence:** `V3-1A — INTERACTIVE-WORKER-SUBSTRATE-1` → `V3-1B — KERNEL-OF-ONE-1` → role-scoped worker capability broker → bounded collaboration and temporary organisation. The first two outcomes precede departments, scheduled autonomy, generic external mutation and the A/B/C organisation comparison.

**Next recommended lane:** `V3-1A — INTERACTIVE-WORKER-SUBSTRATE-1`, currently planned but inactive. It must extend the existing canonical Task → Run authority with provider-native session binding, steering/input, non-terminal controller waiting, explicit-ID recovery, child-process cancellation evidence, raw usage events, sanitised environment and zero worker Soma MCP access. Activation requires explicit owner instruction, a lane-specific plan and any required repository-governance amendment. No V3 implementation lane is active.
