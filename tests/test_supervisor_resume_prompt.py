...
def test_needs_input_prompt_includes_plan_result_and_artifact_references(tmp_path: Path) -> None:
...
def test_blocked_needs_input_prompt_has_blocked_guidance(tmp_path: Path) -> None:
    prompt = build_resume_prompt(supervisor("needs_input", {"blocked": {"reason": "repo_write_lock_unavailable"}}), tmp_path)
...
def test_failed_cancelled_and_completed_guidance(tmp_path: Path) -> None:
    assert "safe recovery plan" in build_resume_prompt(supervisor("failed", error="failed"), tmp_path)
    assert "Confirm cancellation" in build_resume_prompt(supervisor("cancelled"), tmp_path)
    assert "Review the final result" in build_resume_prompt(supervisor("completed"), tmp_path)


def test_prompt_redacts_secret_like_and_truncates_oversized_fields(tmp_path: Path) -> None:
    prompt = build_resume_prompt(
        supervisor("failed", {"hard_stop": {"reason": "API_KEY=abc123", "large": "x" * 3000}}, error="token: secret-value"),
        tmp_path,
    )
...
def test_write_resume_prompt_uses_canonical_path(tmp_path: Path) -> None:
    path = write_resume_prompt(tmp_path / "runs", supervisor("completed"), tmp_path / "runs")
