...
- Live unrestricted-PowerShell replacement smoke evidence (2026-07-17): `typecheck` completed with exit code 0; `vitest` completed with 6 files and 20 tests passed; `frontend_validate` completed with exit code 0. The exact `eslint` replacement reached ESLint 10.6.0 but failed with exit code 2 because Windows denied `scandir` access to the pre-existing `D:/Github/Andia_Beauty/.pytest_cache` directory (`EPERM`). No candidate profile was removed from active configuration because all four replacements have not yet passed.
- Next unit: establish a reviewed non-destructive resolution for the Andia `.pytest_cache` ACL/exclusion failure, rerun the exact `eslint` replacement, and only then remove the four ordinary validation profiles from active external configuration while preserving rollback data.
