...
**Agent 1 (implementer) is complete on 2026-07-28.** The conclusion is **baseline has named measured gaps**. Evidence: [`docs/pilot-memory-1b-shadow-benchmark-2026-07-28.md`](docs/pilot-memory-1b-shadow-benchmark-2026-07-28.md), with the frozen contract in [`docs/pilot-memory-1b-frozen-benchmark-2026-07-28.json`](docs/pilot-memory-1b-frozen-benchmark-2026-07-28.json) and results in [`docs/pilot-memory-1b-results-2026-07-28.json`](docs/pilot-memory-1b-results-2026-07-28.json). The implementation is `soma/pilot_memory_1b/` with focused tests in `tests/test_pilot_memory_1b.py`.

Nine of eleven scored classes reached F1 `1.000`, including multi-hop and partial claim-level supersession, current-fact precision, relations, source/frontmatter drift, deletion, external-edit attribution, and deterministic rebuild, at 62 curated notes and within every predeclared budget at 1,000 and 10,000 notes. The named gap is **semantic paraphrase at F1 `0.333` against a `0.60` threshold**: four of six meaning-based questions returned no correct note at any rank, and a corpus-only local expansion method did not improve them. A second class fails only because one query returned the wrong note *inside the correct project*; an audit found **zero cross-project results**, so structural isolation and unscoped rejection both held. One disclosed correction cycle replaced a fixed top-5 cutoff with R-precision after the original rule proved mathematically unable to meet its own threshold; no threshold was changed and the pre-correction run is preserved. Agent 2 independent adversarial review is now active under [`docs/PILOT_MEMORY_1B_AGENT_1_REVIEW_AND_AGENT_2_BRIEF_2026-07-28.md`](docs/PILOT_MEMORY_1B_AGENT_1_REVIEW_AND_AGENT_2_BRIEF_2026-07-28.md). Controller review confirmed a documented stop condition in the current implementation: relation/referrer traversal is not project-scoped, a synthetic Soma relation can return a sibling-project target, and a Soma-scoped malformed-metadata query can disclose a sibling malformed record. Agent 1's semantic gap remains useful provisional evidence, but commit `7bf955e` is rejected pending a bounded Agent 2 authority/classification/provenance correction with pre-fix evidence preserved. The two-day shadow observation, provider comparison, and production memory remain blocked until Agent 2 returns accept.
